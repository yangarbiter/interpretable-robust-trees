...
class xgboost_wrapper():
    def __init__(self, model, binary=False, classes=None):
        self.model = model 
        self.binary = binary
        self.class_ = classes
    ...
    def predict(self, input_data):
        ...
        if self.binary:
            #test_predict = (test_predict > 0.5).astype(int) * 2 - 1
            test_predict = np.sign(test_predict - 0.5).astype(int)
        else:
...
class xgbKantchelianAttack(object):
    ...
    def __init__(self, model, order=np.inf, guard_val=GUARD_VAL, round_digits=ROUND_DIGITS, LP=False, binary=True, pos_json_input=None, neg_json_input=None):
        ...
        if self.binary :
            ...
            model_dump = model.model.get_dump(dump_format='json')
            model_dump = [json.loads(s) for s in model_dump]
            with open(temp, "w") as fp:
                json.dump(model_dump, fp)
...
def XGBoost_optimal_attack(xgb_classifier, X, y):
    import ipdb
    booster = xgb_classifier.get_booster()
    model = xgboost_wrapper(booster, binary=True)
    #assert (model.predict(X) == xgb_classifier.predict(X)).all(), ipdb.set_trace()

    attack = xgbKantchelianAttack(model=model)
    ret = []
    for xi, yi in zip(X, y):
        ret.append(attack.attack(xi, yi))
    ret = np.asarray(ret)

    #assert np.any(model.predict(ret) == y) == False, ipdb.set_trace()
    return ret
